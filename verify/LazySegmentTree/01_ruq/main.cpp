...
int main() {
   ...
   unsigned int init = ((unsigned int)1 << 31) - 1;
   vector<unsigned int> al(N, init);
   ...
   using M = unsigned int;

   auto eval_op = [](X x, M m) -> X { return m; };
   const M unit_m = numeric_limits<M>::max();  // Mの単位元

   auto lazy_op = [](M m1, M m2) -> M { return m2; };
   auto lazy_p_op = [](M m, size_t n) -> M { return m; };  // operがsumの場合は m * nとする, min/maxは区間に依存しないのでmを返す, XORはnが偶なら0, 奇ならm
   ...
   rep(q, Q) {
      ...
      if (t == 0) {
         ...
         ll ans = laz_seg_tree.query(i, i + 1);
         cout << ans << endl;
      }
   }
