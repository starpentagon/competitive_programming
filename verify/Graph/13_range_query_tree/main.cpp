...
int main() {
   ...
   vector<int> node_parent(N + 1, -1);
   map<pair<int, int>, int> edge_ind_map;
   ...
   rep(i, N) {
      ...
      rep(j, k) {
         ...
         edge_ind_map[{i + 1, c + 1}] = edge_list.size();
         edge_list.emplace_back(i + 1, c + 1, 0);
         node_parent[c + 1] = i + 1;
      }
   }
   ...
   rep(q, Q) {
      ...
      if (t == 0) {
         ...
         int p = node_parent[v];
         int ind = edge_ind_map[{p, v}];

         ll cur_w = euler_tour.GetEdgeWeight(ind);

         euler_tour.UpdateEdgeWeight(ind, cur_w + w);
      } else {
         int u;
         cin >> u;
         u++;

         ll ans = euler_tour.PathQuery(u);
         cout << ans << endl;
      }
   }
